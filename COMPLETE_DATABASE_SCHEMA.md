# Complete Database Schema for Go-Delivery Food Delivery Platform

## Overview
This document provides a comprehensive overview of the database schema for the Go-Delivery food delivery platform. The schema supports a multi-tenant SaaS architecture with features for users, restaurants, orders, menu items, riders, and reviews.

## Table of Contents
1. [Core Tenant System Tables](#core-tenant-system-tables)
2. [User Management Tables](#user-management-tables)
3. [Restaurant Management Tables](#restaurant-management-tables)
4. [Menu Management Tables](#menu-management-tables)
5. [Order Management Tables](#order-management-tables)
6. [Rider Management Tables](#rider-management-tables)
7. [Review & Rating System Tables](#review--rating-system-tables)
8. [Billing & Subscription Tables](#billing--subscription-tables)
9. [Audit & Feature Management Tables](#audit--feature-management-tables)
10. [Entity Relationship Diagram](#entity-relationship-diagram)
11. [Schema Design Notes](#schema-design-notes)

## Core Tenant System Tables

### Tenants
Represents businesses using the platform (multi-tenancy).
...
### UserTenants
Many-to-many relationship between users and tenants with tenant-specific roles.
...
### Restaurants
Represents restaurants within the platform.
...
### Branches
Represents branches of restaurants.
...
### TenantSubscriptions
Active subscriptions linking tenants to their current plans.
...
### TenantInvoices
Invoice management for tenants.
...
## Audit & Feature Management Tables

### TenantAuditLogs
Comprehensive audit trail for all tenant actions.
...
### Default Categories
```sql
INSERT INTO Categories (name, image, is_global) VALUES 
('Pizza', 'pizza.jpg', true),
('Burgers', 'burger.jpg', true),
('Asian', 'asian.jpg', true),
('Desserts', 'dessert.jpg', true),
('Beverages', 'drinks.jpg', true);
```
...
erDiagram
    ...
    Users ||--o{ UserTenants : has
    Tenants ||--o{ Restaurants : has
    Tenants ||--o{ MenuItems : has
    Tenants ||--o{ Orders : has
    Tenants ||--o{ Riders : has
    Tenants ||--o{ RatingsAndReviews : has
    Tenants ||--o{ Categories : has
    Tenants ||--o{ TenantBilling : has
    Tenants ||--o{ TenantSubscriptions : has
    Tenants ||--o{ TenantInvoices : has
    Tenants ||--o{ TenantAuditLogs : has
    Tenants ||--o{ TenantFeatureFlags : has
    SubscriptionPlans ||--o{ TenantSubscriptions : defines
    TenantSubscriptions ||--o{ TenantInvoices : generates
    Users ||--o{ Restaurants : manages
    Users ||--o{ Orders : places
    Users ||--o{ Riders : is
    Users ||--o{ RatingsAndReviews : writes
    Restaurants ||--o{ Branches : has
    Restaurants ||--o{ MenuItems : has
    Restaurants ||--o{ Orders : receives
    Restaurants ||--o{ RatingsAndReviews : receives
    MenuItems ||--o{ OrderItems : included_in
    MenuItems ||--o{ RatingsAndReviews : receives
    Orders ||--o{ OrderItems : contains
    Orders ||--o{ RatingsAndReviews : generates
    Riders ||--o{ Orders : delivers
    Riders ||--o{ RatingsAndReviews : receives
```

## Schema Design Notes

### 1. Multi-Tenant Architecture
- Each business is a "tenant" with its own subdomain
- All data is scoped to tenants for data isolation
...
- Manages current order count

### 8. RatingsAndReviews Table
- Flexible review system supporting reviews for restaurants, menu items, and riders
- Rating constraint ensures values between 1-5

### 9. Billing & Subscription System
- Tenants subscribe to plans with different feature sets
- Billing information stored separately
- Invoicing system for payment tracking

### 10. Audit & Feature Management
- Comprehensive audit trail for compliance
- Feature flags for gradual rollouts and A/B testing
- Tenant-specific configurations

### 11. Relationships
- One-to-many: Tenant -> Restaurants
...
- Many-to-many: Users <-> Tenants (via UserTenants)
